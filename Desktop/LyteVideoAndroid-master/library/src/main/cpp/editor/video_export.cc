...
VideoExport::VideoExport(JNIEnv* env, jobject object)
    ...
    // Because encoder_render cannot change the vertex and texture coordinates
    // And the image read by glReadPixels is upside down
    // So the texture coordinates shown here are rotated by 180 degrees
    // to ensure that the data read from glReadPixels is not upside down, but correct
...
//            LOGE("video export status: %d", av_play_context_->status);
            ...
            if (av_play_context_->is_sw_decode) {
                ...
                // When soft coding, hard decoding needs to be mirrored
                if (!media_codec_encode_) {
        ...
        // Call back synthesis progress to the upper layer
    ...
    if (cancel_) {
        ...
        // Notify java layer synthesis is complete
        OnExportComplete();
    }
    LOGI("leave %s", __func__);
}
