...
import com.reelvideos.app.recorder.fragment.*
import com.reelvideos.app.recorder.view.LineProgressView
import com.reelvideos.app.recorder.view.RecordButton
import com.reelvideos.app.recorder.view.foucs.AutoFocusTrigger
import com.reelvideos.app.recorder.view.foucs.DefaultAutoFocusMarker
import com.reelvideos.app.recorder.view.foucs.MarkerLayout
...
class RecordActivity : AppCompatActivity(), OnRecordingListener, OnRenderListener, RecordButton.OnGestureListener,
    ...
    private val mFlashImage = arrayOf(
    ...
    private var isRateBarVisible = false
    private lateinit var controlLayout: RelativeLayout
    private lateinit var galleryLayout: RelativeLayout
    private lateinit var filtersLayout: RelativeLayout
    private lateinit var doneLayout: RelativeLayout
    ...
    private var selectedMUSIC = ""
    private var selectedMUSICNAME = ""
    private var isMusicSelected = false

    private var finalAudioFileName = ""
    private var convertedAudioFileName = ""
...
//    openLog()
        ...
        galleryLayout = findViewById(R.id.relativeLayout2)
        filtersLayout = findViewById(R.id.relativeLayout3)
        doneLayout = findViewById(R.id.relativeLayout)
        ...
        var recordButton = findViewById<ImageView>(R.id.record_button)
        var recordingButton = findViewById<ImageView>(R.id.recording_button)
        group = findViewById(R.id.rate_bar)
        musicBtn = findViewById(R.id.music)




        recordButton.setOnClickListener(View.OnClickListener { view ->
          if (!isRecording) {
            isRecording = true
            musicBtn.visibility = View.INVISIBLE
            controlLayout.visibility = View.INVISIBLE
            galleryLayout.visibility = View.INVISIBLE
            filtersLayout.visibility = View.INVISIBLE
            doneLayout.visibility = View.INVISIBLE
            group.visibility = View.INVISIBLE
            recordButton.visibility = View.INVISIBLE
            recordingButton.visibility = View.VISIBLE
            isRateBarVisible = false
            onDown()
          }
        })

        recordingButton.setOnClickListener(View.OnClickListener { view ->
          if (isRecording) {
        ...
        findViewById<View>(R.id.show_rate_bar)
                .setOnClickListener {
                    if (!isRateBarVisible) {
                        group.visibility = View.VISIBLE
                        isRateBarVisible = true
                    } else if (isRateBarVisible) {
                        group.visibility = View.INVISIBLE
                        isRateBarVisible = false
                    }
                }

        musicBtn.setOnClickListener(View.OnClickListener { view ->
          showMusic()
        })
        ...
        findViewById<View>(R.id.backBtn)
                .setOnClickListener {
                    onBackPressed()
                }
                ...
                .setOnClickListener {
                    if (mMedias.isNotEmpty()) {
                        ...
                        if (isMusicSelected) {
                            ...
                            intent.putExtra("recordedMusicName", selectedMUSICNAME)
                            intent.putExtra("recordedMusicPath", finalAudioFileName)
                        }
...
//      closeBottomSheet()
        ...
        if (intent.hasExtra("sound_name")) {
            /*  Variables.Selected_sound_id = intent.getStringExtra("sound_id")
              setMusic(Variables.app_hided_folder + Variables.SelectedAudio_AAC);
         */
        }
    ...
    private fun showEffect() {
        ...
        if (effectFragment == null) {
            ...
            supportFragmentManager.commit {
                replace(
                        R.id.frame_container, effectFragment,
                        EFFECT_TAG
                )
            }
        }
              ...
              override fun onIdentifyClick(effect: Effect?) {
                if (effect == null) {
                  ...
                  if (effectPath == "effect/spaceBear") {
                    //Sticker width, 0~1.0
                    //
                    //Parameter explanation: spaceBear for effectName, Is in the parsing effect array type for intoSticker Neutral name
    ...
    private fun showBeauty() {
        ...
        if (beautyFragment == null) {
            ...
            supportFragmentManager.commit {
                replace(
                        R.id.frame_container, beautyFragment,
                        BEAUTY_TAG
                )
            }
        }
    ...
    private fun showFilter() {
        ...
        if (filterFragment == null) {
            ...
            supportFragmentManager.commit {
                replace(
                        R.id.frame_container, filterFragment,
                        FILTER_TAG
                )
            }
        }
    ...
    private fun showMedia() {
        ...
        supportFragmentManager.commit {
            replace(
                    R.id.frame_container, mediaFragment,
                    MEDIA_TAG
            )
        }
        val behavior = BottomSheetBehavior.from(mInsideBottomSheet)
...
//    }
        ...
        startActivityForResult(Intent(this@RecordActivity, SelectMusic::class.java), 1)
    }
    ...
    private fun showSetting() {
        ...
        if (settingFragment == null) {
            ...
            supportFragmentManager.commit {
                replace(
                        R.id.frame_container, settingFragment,
                        SETTING_TAG
                )
            }
        }
...
//        }
    ...
    override fun onDown() {
        ...
        if (mediaPlayer != null && duration < mRecordDuration) {
            mediaPlayer?.start()
        }
...
//                Utils.downloadAudio(this, selectedMUSIC, "temp");
//                Toast.makeText(getApplicationContext(), finalAudioFileName, Toast.LENGTH_LONG).show();
        if (mediaPlayer != null) {
            mediaPlayer!!.pause()
//      mediaPlayer!!.release()
//      mediaPlayer = null
...
//  }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        //getting the music from Select Music Activity and passing it Trinity and MediaPlayer
        if (requestCode == 1 && resultCode == RESULT_OK && data != null) {
            selectedMUSIC = data.getStringExtra("SELECTED_MUSIC").toString()
            selectedMUSICNAME = data.getStringExtra("SELECTED_MUSIC_NAME").toString()
            musicBtn.setText(selectedMUSICNAME)
            musicBtn.startAnimation(AnimationUtils.loadAnimation(this, R.anim.text_animation))
            if (selectedMUSIC.isNotEmpty()) {
                musicBtn.text = selectedMUSICNAME
                musicBtn.setSelected(true)
                isMusicSelected = true
                /*
                            Download Music to Merge

                 */

                finalAudioFileName = Constants.APP_SHOWING_DIR + "SHARED_" + "temp" + ".mp3"
                ...
                convertedAudioFileName = Constants.APP_SHOWING_DIR + "SHARED_" + "working" + ".mp3"
//        Utils.downloadAudio(this, selectedMUSIC, "temp");
//        Toast.makeText(getApplicationContext(), finalAudioFileName, Toast.LENGTH_LONG).show();
                if (mediaPlayer != null) {
                    mediaPlayer!!.stop()
...
//        mediaPlayer.setDataSource(finalFileName);
                    ...
                    mediaPlayer!!.prepare()
                    //findViewById<View>(R.id.music).setText(selectedMUSICNAME)
                } catch (e: IOException) {
                    e.printStackTrace()
                }
            }
    ...
    override fun onResume() {
        ...
        try {
            ...
            mRecordDuration = (preferences.getString("record_duration", "30000") ?: "60000").toInt()
        } catch (e: Exception) {
            e.printStackTrace()
        }
...
//    closeLog()
    ...
    override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences, key: String) {
        when (key) {
          ...
          "record_duration" -> {
            val recordDuration = sharedPreferences.getString(key, "30000") ?: "60000"
    ...
    override fun onBackPressed() {
        ...
        if (behavior.state == BottomSheetBehavior.STATE_EXPANDED) {
            ...
            finish()
            intent = Intent(this@RecordActivity, MainActivity::class.java)
            startActivity(intent)
            super.onBackPressed()
        }
    }
