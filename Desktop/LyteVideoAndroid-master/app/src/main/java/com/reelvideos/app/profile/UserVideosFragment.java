...
import com.reelvideos.app.databinding.FragmentUserVideosBinding;
...
import static com.reelvideos.app.config.Constants.API_FETCH_USER_VIDEOS;
...
public class UserVideosFragment extends BaseFragment {
    ...
    FragmentUserVideosBinding binding;
    ...
    public UserVideosFragment(boolean isExplore, boolean isOwnProfile, String userID) {
    ...
    private void processVideoDelete(String videoID, int pos){
        ...
        binding.loaderanimationforuservideos.playAnimation();
            ...
            public void run() {
                ...
                callAuthAPI(API_DELETE_VIDEO.replace("%video_id%", videoID), Request.Method.DELETE, new ApiResponseCallback() {
                    ...
                    public void onApiSuccessResult(JSONObject jsonObject) {
                        ...
                        binding.loaderanimationforuservideos.setVisibility(View.GONE);
    ...
    private void loadUpVideos() {
        //initially fetches data
        Handler handler = new Handler(Looper.getMainLooper());
        handler.post(new Runnable() {
            @Override
            public void run() {

                callNonAuthAPI(API_FETCH_USER_VIDEOS.replace("%id%", userID), 0, new ApiResponseCallback() {
                    @Override
                    public void onApiSuccessResult(JSONObject jsonObject) {
    ...
    private void loadMoreVideos() {
        //fetches data for every call after initial call
        binding.loaderanimationforuservideos.setVisibility(View.VISIBLE);
        binding.loaderanimationforuservideos.playAnimation();
        Handler handler = new Handler(Looper.getMainLooper());
        handler.post(new Runnable() {
            @Override
    ...
    void processData(JSONObject jsonObject){
        ...
        binding.progressBarUserFrag.setVisibility(View.VISIBLE);
        binding.progressBarUserFrag.animate();
        //dataList.clear();
        ...
        try {
            ...
            adapter.notifyDataSetChanged();
            binding.progressBarUserFrag.setVisibility(View.INVISIBLE);
    ...
    void openWatchVideo(int position){
        ...
        intent.putExtra("isFromProfile", isOwnProfile);
